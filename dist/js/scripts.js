...
if (document.querySelector('.block-reviews__slider')) {
  ...
  function updateSlidesOpacity() {
    const container = document.querySelector('.block-reviews .container');
    if (!container) return;
    ...
    slides.forEach(slide => {
      ...
      const isIntersecting = slideRight > containerLeft && slideLeft < containerRight;
  ...
  window.addEventListener('resize', function () {
    ...
    resizeTimeout = setTimeout(function () {
      if (swiperReviews && !swiperReviews.destroyed) {
        updateSlidesOpacity();
      }
    }, 250);
  });
...
if (fileInput) {
  const fileResultsContainer = document.querySelector('.form-file-results');
  const resultsWrapper = document.querySelector('.form-file-results');
  let selectedFiles = [];

  if (fileResultsContainer) {
    fileResultsContainer.style.display = 'none';
  }
  ...
  fileInput.addEventListener('change', function (e) {
    ...
    if (selectedFiles.length > 0) {
      fileResultsContainer.style.display = 'flex';
      fileResultsContainer.style.flexWrap = 'wrap';
      fileResultsContainer.style.gap = '10px';
    }
  ...
  function addFileToList(file) {
    ...
    resultElement.className = 'form-file-results__result';
    resultElement.setAttribute('data-filename', file.name);

    const nameElement = document.createElement('div');
    nameElement.className = 'form-file-results__name';
    nameElement.textContent = file.name;

    const closeElement = document.createElement('div');
    closeElement.className = 'form-file-results__close';
    ...
    resultElement.appendChild(closeElement);
    fileResultsContainer.appendChild(resultElement);
  }

  function removeFile(filename) {
    selectedFiles = selectedFiles.filter(file => file.name !== filename);

    const resultElement = document.querySelector(`.form-file-results__result[data-filename="${filename}"]`);
    if (resultElement) {
      resultElement.remove();
    }

    if (selectedFiles.length === 0 && fileResultsContainer) {
      fileResultsContainer.style.display = 'none';
    }
  }
}
  ...
  if (spollersArray.length > 0) {
    ...
    function setSpollerAction(e) {
      ...
      if (el.closest("[data-spoller]")) {
        ...
        const spollerItem = spollerTitle.closest(".spollers-faq__item");
        ...
        if (!spollersBlock.querySelectorAll("._slide").length) {
          ...
          // Добавляем/убираем класс _spoller-active у родителя
          if (spollerItem) {
    ...
    function hideSpollersBody(spollersBlock) {
      ...
      if (spollerActiveTitle && !spollersBlock.querySelectorAll("._slide").length) {
        const spollerItem = spollerActiveTitle.closest(".spollers-faq__item");
      ...
      document.addEventListener("click", (function (e) {
        ...
        if (!el.closest("[data-spollers]")) {
          spollersClose.forEach((spollerClose => {
            ...
            const spollerItem = spollerClose.closest(".spollers-faq__item");
            if (spollerItem) spollerItem.classList.remove("_spoller-active");
